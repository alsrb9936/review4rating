...
class SGDNDataset(RecDataset):
    """Full-graph SGDN dataset with encoder/decoder split.

    Follows RGCLDataset pattern:
    - Encoder graph: train interactions only (used for message passing)
    - Decoder edges: all interactions in the current split (used for prediction)
    - Review features: from cached review_embedding column
    """
    ...
    def _build_decoder_tensors(self, frame) -> None:
        ...
        self.labels = torch.tensor(
            np.searchsorted(np.array(self.rating_vals, dtype=np.float32), ratings), dtype=torch.long
        )
