...
SCG_YAML_PATH = REPO_ROOT / "config" / "yaml" / "scg_rgcl.yaml"
...
SEARCH_SPACE = {
    ...
    "model.min_gate": [0.05, 0.10, 0.15, 0.20],
    "model.gate_beta0_init": [1.0, 2.0, 3.0],
    "model.gate_beta1_init": [2.0, 4.0, 6.0],
}
...
def patch_scg_yaml(base_config: dict, params: dict):
    config = deepcopy(base_config)
    for key, value in params.items():
        set_nested(config, key, value)
    save_yaml(SCG_YAML_PATH, config)
...
def main():
    ...
    if not SCG_YAML_PATH.exists():
        raise FileNotFoundError(f"Cannot find {SCG_YAML_PATH}")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = REPO_ROOT / "results" / "greedy_search_scg_rgcl" / timestamp
    out_dir.mkdir(parents=True, exist_ok=True)

    csv_path = out_dir / "search_log.csv"
    best_path = out_dir / "best_config.json"

    original_yaml_text = SCG_YAML_PATH.read_text(encoding="utf-8")
    base_config = load_yaml(SCG_YAML_PATH)
    ...
    try:
        patch_scg_yaml(base_config, best_params)
        ...
        for round_idx in range(1, args.max_rounds + 1):
            ...
            for param_name, candidates in SEARCH_SPACE.items():
                ...
                for value in candidates:
                    ...
                    patch_scg_yaml(base_config, trial_params)
        ...
        print("\n[INTERRUPTED] Restoring original scg_rgcl.yaml")
        SCG_YAML_PATH.write_text(original_yaml_text, encoding="utf-8")
        raise

    except Exception:
        print("\n[ERROR] Restoring original scg_rgcl.yaml")
        SCG_YAML_PATH.write_text(original_yaml_text, encoding="utf-8")
        raise
